...
<p>The script assumes the use of <a href="https://swiftmailer.symfony.com/docs/introduction.html" target="_blank">Swift Mailer</a>.</p>
...
// FILL $to ARRAY WITH NAMES AND ADDRESSES FROM DATABASE
$to = [];
while ( $row = $result->fetch_object() )
{
	$to[] = ['name' => $row->name, 'address' => $row->email, 'token' => $row->token];
}
...
$sendFrom            = ['info@yourdomain.com' => 'Administrator'];
$voteurl             = $_SERVER["HTTP_HOST"] .  dirname($_SERVER["SCRIPT_NAME"]) . '/vote.php?token=';
...
foreach ( $to as $key => $value )
{
	...
	$votelink = $voteurl . $value['token'];
	$votehref = '<a href="vote.php?token=' . $value['token'] . '">' . $votelink . '</a>';
